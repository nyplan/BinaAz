...
public class UpdateOfficeCommandHandler : IRequestHandler<UpdateOfficeCommandRequest, UpdateOfficeCommandResponse>
{
    ...
    public UpdateOfficeCommandHandler(IItemService itemService, ILocalStorageService localStorageService, IRepository<Domain.Entities.TPH.Base.Item> itemRepository, IHttpContextAccessor contextAccessor)
    {
    ...
    public async Task<UpdateOfficeCommandResponse> Handle(UpdateOfficeCommandRequest request, CancellationToken cancellationToken)
    {
        ...
        var newItem = await _itemService.UpdateItem<Office>(request.Dto, request.ItemNumber);
        newItem.Id = item.Id;
