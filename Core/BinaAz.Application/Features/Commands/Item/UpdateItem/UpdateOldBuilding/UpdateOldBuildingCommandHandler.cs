...
public class UpdateOldBuildingCommandHandler : IRequestHandler<UpdateOldBuildingCommandRequest,UpdateOldBuildingCommandResponse>
{
    ...
    public UpdateOldBuildingCommandHandler(IItemService itemService, ILocalStorageService localStorageService, IRepository<Domain.Entities.TPH.Base.Item> itemRepository, IHttpContextAccessor contextAccessor)
    {
    ...
    public async Task<UpdateOldBuildingCommandResponse> Handle(UpdateOldBuildingCommandRequest request, CancellationToken cancellationToken)
    {
        ...
        var newItem = await _itemService.UpdateItem<OldBuilding>(request.Dto, request.ItemNumber);
        newItem.Id = item.Id;
