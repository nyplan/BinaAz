...
public class UpdateGarageCommandHandler : IRequestHandler<UpdateGarageCommandRequest, UpdateGarageCommandResponse>
{
    ...
    public UpdateGarageCommandHandler(IItemService itemService, ILocalStorageService localStorageService, IRepository<Domain.Entities.TPH.Base.Item> itemRepository, IHttpContextAccessor contextAccessor)
    {
    ...
    public async Task<UpdateGarageCommandResponse> Handle(UpdateGarageCommandRequest request, CancellationToken cancellationToken)
    {
        ...
        var newItem = await _itemService.UpdateItem<Garage>(request.Dto, request.ItemNumber);
        newItem.Id = item.Id;
