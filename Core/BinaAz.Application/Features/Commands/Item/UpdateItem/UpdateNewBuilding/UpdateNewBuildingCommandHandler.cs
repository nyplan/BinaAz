...
public class UpdateNewBuildingCommandHandler : IRequestHandler<UpdateNewBuildingCommandRequest, UpdateNewBuildingCommandResponse>
{
    ...
    public UpdateNewBuildingCommandHandler(IItemService itemService, ILocalStorageService localStorageService, IRepository<Domain.Entities.TPH.Base.Item> itemRepository, IHttpContextAccessor contextAccessor)
    {
    ...
    public async Task<UpdateNewBuildingCommandResponse> Handle(UpdateNewBuildingCommandRequest request, CancellationToken cancellationToken)
    {
        ...
        var newItem = await _itemService.UpdateItem<NewBuilding>(request.Dto, request.ItemNumber);
        newItem.Id = item.Id;
