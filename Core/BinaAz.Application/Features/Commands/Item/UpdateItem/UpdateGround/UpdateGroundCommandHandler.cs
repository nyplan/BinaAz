...
public class UpdateGroundCommandHandler : IRequestHandler<UpdateGroundCommandRequest, UpdateGroundCommandResponse>
{
    ...
    public UpdateGroundCommandHandler(IItemService itemService, ILocalStorageService localStorageService, IRepository<Domain.Entities.TPH.Base.Item> itemRepository, IHttpContextAccessor contextAccessor)
    {
    ...
    public async Task<UpdateGroundCommandResponse> Handle(UpdateGroundCommandRequest request, CancellationToken cancellationToken)
    {
        ...
        var newItem = await _itemService.UpdateItem<Ground>(request.Dto, request.ItemNumber);
        newItem.Id = item.Id;
