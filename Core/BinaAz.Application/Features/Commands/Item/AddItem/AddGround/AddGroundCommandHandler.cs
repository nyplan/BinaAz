...
public class AddGroundCommandHandler : IRequestHandler<AddGroundCommandRequest, AddGroundCommandResponse>
{
    ...
    public AddGroundCommandHandler(IItemService itemService, ILocalStorageService localStorageService, IRepository<Image> imageRepository, IRepository<Domain.Entities.TPH.Base.Item> itemRepository, IHttpContextAccessor contextAccessor)
    {
    ...
    public async Task<AddGroundCommandResponse> Handle(AddGroundCommandRequest request, CancellationToken cancellationToken)
    {
        var item = await _itemService.MapToItem<Ground>(request.Dto);
