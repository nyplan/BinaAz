...
public class UserService : IUserService
{
    ...
    public async Task<bool> CreateAgencyAsync(CreateAgency model)
    {
        if (model.Password != model.PasswordConfirm)
            throw new Exception("Passwords does not match");
        ...
        if (model.Email is not null)
        {
            if (_userRepository.Table.Any(c => c.Email == model.Email))
                throw new Exception("This email already exist");
            await _userRepository.AddAsync(new Agency()
            {
                Email = model.Email,
                HashedPassword = hashedPassword[0],
                Salt = hashedPassword[1],
                CompanyName = model.CompanyName,
                RelevantPerson = model.RelevantPerson
            });
        }
    ...
    public async Task<bool> CreateResidentialComplexAsync(CreateResidentialComplex model)
    {
        if (model.Password != model.PasswordConfirm)
            throw new Exception("Passwords does not match");
        ...
        if (model.Email is not null)
        {
            ...
            await _userRepository.AddAsync(new Agency()
            {
                ...
                HandOverYear = model.HandOverYear
            });
        }
